...
import duckdb  # Replaces deltalake
...
class Config:
    ...
    OCI_BUCKET = os.environ.get('OCI_BUCKET', 'oci-s3-ykg-storage')
    ...
    # OCI S3 Compatibility Endpoint Format: 
...
def extract_to_arrow(ds, target_var):
    ...
    # Time normalization (Force Microseconds to natively match analytics engines)
    ...
    # CAST TO STRING: Great for Parquet partitioning
    table = table.set_column(
...
# --- REPLACED: DELTA LAKE ENGINE WITH DUCKDB GOLD ENGINE ---
def write_gold_parquet_duckdb(pa_table, pk_cols, target_s3_path, storage_options):
    """
    Directly writes PyArrow tables to Partitioned Parquet on OCI via DuckDB.
    Replaces Delta Lake 'upsert_weather_data'.
    """
    logger.info(f"♻️ Executing DuckDB Partition Overwrite to: {target_s3_path}")
    
    # Clean the path to ensure valid DuckDB syntax
    target_path = target_s3_path.rstrip('/')
    
    # Connect DuckDB in-memory engine
    con = duckdb.connect(database=':memory:')
    
    try:
        # Load AWS Extension and bind OCI S3-Compatibility parameters
        ...
        # Format the endpoint for DuckDB (strip http:// or https://)
        endpoint = storage_options['AWS_ENDPOINT_URL'].replace('https://', '').replace('http://', '')
        con.execute(f"SET s3_endpoint='{endpoint}';")
        
        # Required for OCI Object Storage compatibility
        con.execute("SET s3_url_style='path';") 
        con.execute("SET s3_use_ssl=true;")
        
        # Register the Arrow Table as a virtual table in DuckDB
        con.register('arrow_view', pa_table)
        
        # Write Hive-Partitioned Parquet. 
        # OVERWRITE_OR_IGNORE 1 replaces the entire partition atomically.
        sql = f"""
            COPY (SELECT * FROM arrow_view) 
            TO '{target_path}' 
            (FORMAT PARQUET, PARTITION_BY (forecast_date, forecast_cycle), OVERWRITE_OR_IGNORE 1, COMPRESSION 'ZSTD')
        """
        con.execute(sql)
        logger.info(f"🎉 DuckDB Pipeline Transaction Complete! Gold layer updated.")
        
    except Exception as e:
        logger.error(f"❌ DuckDB Gold Export Failed: {e}")
        raise
    finally:
        con.close()
...
def download_gfs_robust(date_obj, cycle, steps):
    """GFS Downloader (GRIB2 -> Xarray -> PyArrow -> Gold Parquet on OCI)"""
    ...
    # ---------------------------------------------------------
    # BATCH WRITE TO GOLD PARQUET (OCI Storage via DuckDB)
    # ---------------------------------------------------------
    if arrow_tables and master_pk_cols:
        try:
            logger.info(f"📦 Batching {len(arrow_tables)} GFS steps into a single OCI DuckDB transaction...")
            master_table = pa.concat_tables(arrow_tables)
            
            # Swapped /delta_lake/ for /gold/
            target_s3_path = f"s3://{weather_config.OCI_BUCKET}/weather_data/gold/gfs_raw/"
            ...
            storage_options = {
                ...
                "AWS_ENDPOINT_URL": weather_config.OCI_ENDPOINT_URL
            }
            write_gold_parquet_duckdb(master_table, master_pk_cols, target_s3_path, storage_options)
        except Exception as e:
...
def download_ecmwf_unified(date_obj, cycle, steps, target_model='aifs', task_type='upper'): 
    """ECMWF Unified Downloader (GRIB2 -> Xarray -> PyArrow -> Gold Parquet on OCI)"""
            ...
            task_dict = {
                ...
                "params": {
                    **common_params, 
                    ...
                    "param": list(set(short_names_upper))
                }
            }
    ...
    if arrow_tables and master_pk_cols and task_name:
        try:
            logger.info(f"📦 Batching {len(arrow_tables)} ECMWF steps into a single OCI DuckDB transaction...")
            master_table = pa.concat_tables(arrow_tables)
            
            # Swapped /delta_lake/ for /gold/
            target_s3_path = f"s3://{weather_config.OCI_BUCKET}/weather_data/gold/ecmwf_raw/{task_name}/"
...
def run_dbt_command(command: str, select_path: str = None):
    """
    Standardized runner to execute dbt commands in the 'dbt-oracledw-runner' container.
    """
    client = docker.from_env()
    try:
        # Changed from dbt-snowflake-runner to dbt-oracledw-runner
        container = client.containers.get('dbt-oracledw-runner')
        
        full_cmd = f"dbt {command} --profiles-dir . --target prod_oracledw"
        if select_path:
        ...
        raise Exception("Container 'dbt-oracledw-runner' not found.")