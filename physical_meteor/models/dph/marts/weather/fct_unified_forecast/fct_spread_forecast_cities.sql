...
SELECT 
    md5(concat_ws('-', f.weather_model, f.cycle_date::text, f.cycle_hour::text, f.forecast_step_hours::text, f.pressure_level_hpa::text, f.lat_i::text, f.lon_i::text)) as unique_key,
    f.*,
    d.city_name,
    d.state,
    d.region,
    d.market_weight,
    d.associated_hub
FROM {{ ref('fct_spread_forecast') }} f
